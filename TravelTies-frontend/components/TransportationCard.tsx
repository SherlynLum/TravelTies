import { View, Text, Dimensions, TouchableOpacity, Alert, Linking } from 'react-native'
import React, { useEffect, useState } from 'react'
import { Link } from 'expo-router';
import { Card } from 'react-native-paper';
import { TransportationCardPreview } from '@/types/cards';
...
const TransportationCard = ({tab, tripId, _id, title, startDate, startTime, endDate, endTime, 
    departureAddress, arrivalAddress, removeFromTab} : TransportationCardPreview) => {
    ...
    const viewAddress = async (address: string | undefined, type: string) => {
        if (!address) {
            Alert.alert("View on map", `No ${type} address is provided for card: ${title}`);
            return;
        }
        try {
            setViewLoading(true);
            ...
            console.log(e);
            Alert.alert("View on map", `Failed to open Google Maps with this ${type} address - please try again`);
        } finally {
            ...
            <View className="pb-5" style={{paddingHorizontal: 18}}>
                <Card style={{padding: 0, borderRadius: 16, width: coverWidth}}>
                    <View className="flex flex-col gap-4">
                        ...
                        {/* card content */}
                        {(arrivalAddress || departureAddress) ? (
                            <View className="flex flex-row px-3 justify-between"
                            style={{paddingBottom: 15}}>
                                <View className="flex flex-col gap-2.5 justify-center items-start">
                                    {/* card title */}
                                    <Text className="font-semibold text-lg text-left" numberOfLines={3}
                                    ellipsizeMode="tail">
                                        {title}
                                    </Text>
                                    ...
                                    {viewLoading ? (
                                        ...
                                        <TouchableOpacity onPress={(e) => {
                                            ...
                                            if (departureAddress && arrivalAddress) {
                                                handleViewOnMap();
                                            } else if (departureAddress) {
                                                viewAddress(departureAddress, "departure");
                                            } else if (arrivalAddress) {
                                                viewAddress(arrivalAddress, "arrival");
                                            }
                                        }}
                                        className='flex flex-row gap-2.5 bg-blue-400 justify-center items-center 
                                        border border-blue-700 shadow-sm h-[44px] px-8 rounded-[30px]'>

...
import { GeneralCardPreview } from '@/types/cards';
...
const GeneralCard = ({tab, tripId, _id, cardType, title, startDate, startTime, endDate, endTime, 
    ...
    const handleViewOnMap = async () => {
        if (!generalAddress) {
            Alert.alert("View on map", `No address is provided for card: ${title}`);
            return;
        }
        try {
            setViewLoading(true);
            const url = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(generalAddress)}`;
            ...
            console.log(e);
            Alert.alert("View on map", "Failed to open Google Maps with this address - please try again");
        } finally {

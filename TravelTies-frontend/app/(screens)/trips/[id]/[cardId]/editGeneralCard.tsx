...
const EditGeneralCard = () => {
    ...
    useEffect(() => {
        ...
        const getCurrentCard = async () => {
            try {
                ...
                setCardType(currentCard.cardType);
                if (currentCard.generalAddress) {
                    setGeneralAddress(currentCard.generalAddress);
                }
                ...
                if (currentCard.webUrls.length > 0) {
                    setUrls(currentCard.webUrls.map((webUrl: string) => ({id: uuidv4(), url: webUrl})));
                }
                ...
                console.log(e)
                Alert.alert("Update General card", 
                    "Cannot load this General card",
                    [{
    ...
    useEffect(() => {
        ...
        const getDays = async () => {
            try {
                ...
                if (err.length === 2) {
                    Alert.alert("Edit General card", "Card start date and end date were outside the trip duration and have been reset");
                } else if (err.length === 1) {
                    Alert.alert("Edit General card", err[0]);
                } 
                setDaysErr(false);
            } catch (e) {
                setDayData([]);
                console.log(e);
                Alert.alert("Edit General card", "Unable to load this trip’s duration, so start and end dates cannot be changed");
                if (!startDate) {
    ...
    const uploadImages = async () => {
        try{
            ...
            const combinedPicIds = [...picIds, ...picIdsRes];
            setPicIds(combinedPicIds);
            setUploadPicSuccess(true);
            return {combinedPicIds, hasSomeErr};
        } catch (e) {
            console.log(e);
    ...
    const deleteDoc = (uri: string) => {
        ...
        const key = docsToDisplay.find((doc): doc is DocWithUrl => "url" in doc && doc.url === uri)?.key;
    ...
    const uploadFiles = async () => {
        try{
            ...
            const combinedDocs = [...docs, ...successfulDocs];
            setDocs(combinedDocs);
            setUploadDocSuccess(true);
            return {combinedDocs, hasSomeErr};
        } catch (e) {
            console.log(e);
    ...
    const updateCurrentCard = useCallback(async () => {
        try {
            ...
            if (!uploadPicSuccess && newImages.length !== 0) {
                const uploadPicRes = await uploadImages();
                if (!uploadPicRes) {
                    throw new Error("Failed to update General card due to failure in uploading photos");
                }
                imageIds = uploadPicRes.combinedPicIds;
                imageErr = uploadPicRes.hasSomeErr;
            }
            if (!uploadDocSuccess && newDocs.length !== 0) {
                const uploadDocRes = await uploadFiles();
                if (!uploadDocRes) {
                    throw new Error("Failed to update General card due to failure in uploading documents");
                }
                files = uploadDocRes.combinedDocs;
                filesErr = uploadDocRes.hasSomeErr;
            }
            const card = await updateCard({token, cardId, cardType, title,
            ...
            if (isAxiosError(e) && e.response?.data?.timeErr) {
                Alert.alert("Failed to update General card", e.response.data.message);
                return;
            }
            Alert.alert("Failed to update General card", "Please try again later");
        } finally {
            setLoading(false);
        }
    }, [title, cardType, generalAddress, description, startDate, startTimeStr, endDate,
        endTimeStr, picIds, newImages, docs, newDocs, urls])
    
    const handleUpdate = useCallback(async () => {
        if (!title) {
            Alert.alert("Failed to update General card", "Title cannot be empty");
            return;
        }
        if (!cardType) {
            Alert.alert("Failed to update General card", "General card must specify a card type");
            return;
        }
        if (startDate && endDate) {
            if (endDate < startDate) {
                Alert.alert("Failed to update General card", "End date must be after start date");
                return;
            }
            if (startDate === endDate && startTimeStr && endTimeStr && endTimeStr < startTimeStr) {
                Alert.alert("Failed to update General card", 
    ...
    }, [title, cardType, startDate, endDate, startTimeStr, endTimeStr, updateCurrentCard])

    ...
    useEffect(() => {
        ...
        const getCurrentCard = async () => {
            try {
                ...
                setCountry(currentCard.country);
                if (currentCard.city) {
                    setCity(currentCard.city);
                }
                ...
                console.log(e)
                Alert.alert("Update Destination card", 
                    "Cannot load this Destination card",
                    [{
    ...
    useEffect(() => {
        ...
        const getDays = async () => {
            try {
                ...
                if (err.length === 2) {
                    Alert.alert("Edit Destination card", "Card start date and end date were outside the trip duration and have been reset");
                } else if (err.length === 1) {
                    Alert.alert("Edit Destination card", err[0]);
                } 
                setDaysErr(false);
            } catch (e) {
                setData([]);
                console.log(e);
                Alert.alert("Edit Destination card", "Unable to load this trip’s duration, so start and end dates cannot be changed");
                if (!startDate) {
    ...
    const updateCurrentCard = useCallback(async () => {
        try {
            ...
            if (!uploadPicSuccess && newImages.length !== 0) {
                ...
                if (!uploadPicRes) {
                    throw new Error("Failed to update Destination card due to failure in uploading photos");
                }
            ...
            if (!uploadDocSuccess && newDocs.length !== 0) {
                ...
                if (!uploadDocRes) {
                    throw new Error("Failed to update Destination card due to failure in uploading documents");
                }
                files = uploadDocRes.combinedDocs;
                filesErr = uploadDocRes.hasSomeErr;
            }
            const card = await updateCard({token, cardId, cardType: "destination", title,
            ...
            if (isAxiosError(e) && e.response?.data?.timeErr) {
                Alert.alert("Failed to update Destination card", e.response.data.message);
                return;
            }
            Alert.alert("Failed to update Destination card", "Please try again later");
        } finally {
            setLoading(false);
        }
    }, [title, country, city, description, startDate, startTimeStr, endDate, endTimeStr, 
        picIds, newImages, docs, newDocs, urls])
    
    const handleUpdate = useCallback(async () => {
        if (!title) {
            Alert.alert("Failed to update Destination card", "Title cannot be empty");
            return;
        }
        if (!country) {
            Alert.alert("Failed to update Destination card", "Destination card must specify a country")
        }
        if (startDate && endDate) {
            if (endDate < startDate) {
                Alert.alert("Failed to update Destination card", "End date must be after start date");
                return;
            }
            if (startDate === endDate && startTimeStr && endTimeStr && endTimeStr < startTimeStr) {
                Alert.alert("Failed to update Destination card", 
...
export default EditDestinationCard